...
在输出每条问题时，按以下标准判断严重程度：

- **高风险**：可能导致资损、用户数据丢失、核心流程阻断、监管合规违规
- **中风险**：影响用户体验或转化，不造成直接损失；或存在争议需产品决策
- **低风险（建议）**：可延后到下一版本的优化项；或 MVP 阶段可接受的已知问题
...
- 🔵 建议类问题酌情处理，如果能简单补充则补充
- 如果用户对评审待确认问题提供了回答，将回答融入文档对应位置
- 对于用户未回答的评审待确认问题，必须自行选择最小可行方案并在文中直接写明决策内容，**不得保留 [待确认] 标记**，可在该段落末尾用括号注明：（已按最简方案处理，如需调整请反馈）
- 对于 PRD 初稿中标注 **[待确认]** 的内容，如果评审意见、用户补充回答或上下文中能推断出合理方案，直接给出建议方案并标注 **[建议方案]**
- 如果确实无法推断，保留 **[待确认]** 标注
